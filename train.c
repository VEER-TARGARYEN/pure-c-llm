...
/* ---- "deep and narrow" architecture ----
 * Every dim is overridable at compile time (e.g. for a Net2Net-grown model):
 *   gcc -DD_MODEL=512 -DN_HEADS=16 -DD_FF=2048 -DN_LAYERS=16 train.c ...     */
#ifndef BLOCK_SIZE
...
/* ---- training shape (fixed at compile time for simple allocation) ---- */
#define B_SIZE 4                       /* batch size                    */
...
/* ==========================================================================
 * CHECKPOINTS  --  weights only, in registry order (config in the header).
 * Format: int[8] { magic, block, d_model, n_heads, d_ff, n_layers, vocab, step }
 * then every parameter tensor's floats, in build_params() registration order.
 * ========================================================================== */
#define CKPT_MAGIC 0x434B5031   /* 'CKP1' */

static void save_ckpt(const char *path, int step) {
    FILE *f = fopen(path, "wb");
    if (!f) { fprintf(stderr, "cannot write %s\n", path); exit(1); }
    int hdr[8] = { CKPT_MAGIC, BLOCK_SIZE, D_MODEL, N_HEADS, D_FF, N_LAYERS, VOCAB_SIZE, step };
    fwrite(hdr, sizeof(int), 8, f);
    for (int p = 0; p < g_nparams; p++)
        fwrite(g_params[p].w, sizeof(float), g_params[p].n, f);
    fclose(f);
    printf("saved checkpoint -> %s\n", path);
}

static int load_ckpt(const char *path) {
    FILE *f = fopen(path, "rb");
    if (!f) { fprintf(stderr, "cannot open %s\n", path); exit(1); }
    int hdr[8];
    if (fread(hdr, sizeof(int), 8, f) != 8 || hdr[0] != CKPT_MAGIC) {
        fprintf(stderr, "%s is not a valid checkpoint\n", path); exit(1);
    }
    if (hdr[1]!=BLOCK_SIZE || hdr[2]!=D_MODEL || hdr[3]!=N_HEADS ||
        hdr[4]!=D_FF || hdr[5]!=N_LAYERS || hdr[6]!=VOCAB_SIZE) {
        fprintf(stderr, "checkpoint config (d=%d h=%d ff=%d L=%d) does not match "
                        "this build (d=%d h=%d ff=%d L=%d)\n",
                hdr[2],hdr[3],hdr[4],hdr[5], D_MODEL,N_HEADS,D_FF,N_LAYERS);
        exit(1);
    }
    for (int p = 0; p < g_nparams; p++)
        if (fread(g_params[p].w, sizeof(float), g_params[p].n, f) != (size_t)g_params[p].n) {
...
/* ==========================================================================
 * MAIN  --  training loop
 *   usage: train [steps] [lr] [ckpt_in | -] [ckpt_out]
 * ========================================================================== */
int main(int argc, char **argv) {
    ...
    const char *ckpt_in  = (argc > 3 && strcmp(argv[3], "-") != 0) ? argv[3] : NULL;
    const char *ckpt_out = (argc > 4) ? argv[4] : "ckpt.bin";
    if (ckpt_in) {
        int s0 = load_ckpt(ckpt_in);
        printf("resumed weights from %s (saved at step %d)\n", ckpt_in, s0);
    }
